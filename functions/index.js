...
exports.moderateMessage = onRequest(
  ...
  async (req, res) => {
    ...
    for (const p of patterns) {
      if (p.re.test(text)) return res.json({ decision: 'BLOCK', reason: p.reason, violationType: p.violationType });
    }
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          system: [{ type: 'text', text: 'Strict multilingual chat moderator for Indian rental platform. Respond ONLY with valid JSON.' }],
          messages: [{ role: 'user', content: prompt }]
        })
      });
      const data = await response.json();
      const raw = data?.content?.[0]?.text || '{"decision":"ALLOW","reason":"","violationType":""}';
      return res.json(JSON.parse(raw.replace(/```json|```/g, '').trim()));
    } catch (e) {
      console.error('Moderation error:', e);
      return res.json({ decision: 'ALLOW', reason: '', violationType: '' });
    }
  }
);
